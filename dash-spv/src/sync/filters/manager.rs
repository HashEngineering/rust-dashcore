        ...
        while let Some((&batch_start, batch)) = self.active_batches.first_key_value() {
            ...
            if !batch.rescan_complete() {
                ...
                if !scripts_by_wallet.is_empty() {
                    ...
                    // and sweep once this batch reaches its forward fixpoint,
                ...
                // Forward direction quiescent for this batch: one combined
                // backward sweep over the committed range with everything
                // accumulated (#846). Hits attribute to this batch, so
                // scripts their processing derives re-enter through
                // `collected_scripts` above and only genuinely new scripts
                // get a follow-up sweep. Runs before the probe ladder: the
                // sweep can resume the chase; the probe stays the terminal
                // check.
                ...
                if !backward_scripts.is_empty() {
                    events.extend(
                        self.rescan_committed_range(batch_start, &backward_scripts).await?,
                    );
